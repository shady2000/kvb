...
class KeyValueBottleneck(nn.Module):
    def __init__(
        ...
        super().__init__()
        init_mode_list = ["random", "kmeans", "ours"]
        assert init_mode in init_mode_list
        init_id = init_mode_list.index(init_mode)
        ...
        if verbose:
            print("Line 109, size of self.keys:", self.keys.size()) #256 x num_keys x 14
        self.class_idx_centroids = []

    @torch.jit.ignore
    def init_embed_(self, data, labels):
        #data shape: (b, c, n, k)
        b, c, n, k = data.size()
        if verbose:
            print("init:--------------------------->", int(self.initted.item()))
        match self.initted:
            case 0:
                return
            case 1: #kmeans normally
                if verbose:
                    print("using normal kmeans")
                    print("data shape:", data.size()) #(b, c, n, k)
                data_flatten = rearrange(data, "b c n k -> c (b n) k")
                #c (b n) k. c = num-codebooks, b = bsz, n = 1 = num_tokens, k = dim_keys
                keys, cluster_size = kmeans(
                    data_flatten, self.key_value_pairs_per_codebook, self.kmeans_iters
                )
                if verbose:
                    print("Line 119, size of self.keys:", self.keys.size()) #256 x num_keys x 14
                self.keys.data.copy_(keys)
                self.keys.data.copy_(keys.clone())
                self.cluster_size.data.copy_(cluster_size)
                self.initted.data.copy_(torch.Tensor([1.0]))
                if verbose:
                    print("Line 124, size of self.keys:", self.keys.size()) #256 x num_keys x 14
            case 2: #kmeans, ours
                if verbose:
                    print("using our kmeans")
                all_keys = None
                cur_start_idx = 0
                num_unique_labels = torch.unique(labels).size()[0]
                num_keys_per_class = self.key_value_pairs_per_codebook//num_unique_labels
                for label in torch.unique(labels):
                    self.class_idx_centroids.append(cur_start_idx)
                    indices = torch.nonzero(labels == label)
                    sliced_data = data[indices, :, :, :].squeeze() #size x num_keys x 14
                    sliced_data_flatten = rearrange(sliced_data, "b c k -> c b k")
                    centroids, cluster_size = kmeans(sliced_data_flatten, num_keys_per_class, self.kmeans_iters)
                    if all_keys is None:
                        all_keys = centroids
                    else:
                        all_keys = torch.cat((all_keys, centroids), dim=1)
                    num_keys = centroids.size()[1]
                    cur_end_idx = cur_start_idx + num_keys
                    label_int = label.item()
                    self.assoc_keys_range[label_int] = (cur_start_idx, cur_end_idx)
                    cur_start_idx = cur_end_idx
                if verbose:
                    print("shape of all_keys is", all_keys.size())
                #self.keys.data = all_keys.clone()

                self.keys.data.copy_(all_keys)
                self.keys.data.copy_(all_keys.clone())
                self.initted.data.copy_(torch.Tensor([2.0]))
            case 3: #OT, ours
                raise NotImplementedError
            case _:
                exit("init mode not supported")
    ...
    def forward(self, x, labels):
        if not self.initted == 2:

            x = x.float()

            # x.shape = b c n k
            if verbose:
                print("shape of x is (b, c, n, k) so it is:", x.size())
            shape, dtype, device = x.shape, x.dtype, x.device
            assert x.ndim == 4
            b, c, n, k = shape

            flatten = rearrange(x, "b c n k -> c (b n) k")
            self.init_embed_(x, labels)
            ...
            if self.update_keys:
                ...
                self.expire_codes_(flatten) #VQ-VAE
            else:
            ...
            x = x.float()
            print("shape of x is (b, c, n, k) so it is:", x.size())
            all_quantized_values = None
            all_quantized_values = None
            all_quantized_keys = None
            all_keys_ind = None
            all_dist_to_fetched_keys = None
            all_counts = None
            # loop over values in the label
            for label in torch.unique(labels):
                sliced_indices = torch.nonzero(labels == label).squeeze()
                sliced_x = x[sliced_indices, :, :] #size x num_keys x 14

                shape, dtype, device = sliced_x.shape, sliced_x.dtype, sliced_x.device
                assert x.ndim == 4
                b, c, n, k = shape
                flatten = rearrange(sliced_x, "b c n k -> c (b n) k")
                self.init_embed_(x, labels)
                ...
                #   key-value pairs.
                label_int = label.item()
                keys_start, keys_end = self.assoc_keys_range[label_int]
                current_keys = self.keys[:,keys_start: keys_end, :]
                dist: torch.Tensor = -(  # noqa
                        torch.einsum("cid,cid->ci", flatten, flatten)[..., None]
                        - 2 * torch.einsum("cid,cjd->cij", flatten, current_keys)
                        + torch.einsum("cjd,cjd->cj", current_keys, current_keys)[:, None, :]
                )
                ...
                # gather_ind_keys.shape = c(i topk)k
                gather_ind_keys = repeat(keys_ind, "c i -> c i k", k=self.dim_keys)
                # current_keys.shape = cjk
                # quantized_keys.shape = c(i topk)k
                #quantized_keys = torch.gather(current_keys, dim=1, index=gather_ind_keys)
                # gather_ind_values.shape = c(i topk)v
                gather_ind_values = repeat(keys_ind, "c i -> c i v", v=self.dim_values)
                # self.values.shape = cjv
                quantized_values = torch.gather(self.values, dim=1, index=gather_ind_values)
                ...
                if self.update_keys:
                    ...
                    flatten = einops.repeat(flatten, "c n k -> c (n topk) k", topk=forward_topk)
                    keys_sum = torch.einsum("cik,cij->ckj", flatten, keys_onehot)
                    current_keys_avg = current_keys
                    ema_inplace(
                        current_keys_avg, rearrange(keys_sum, "c d n -> c n d"), self.decay
                    )
                    # cluster_size.shape = cj
                    cluster_size = laplace_smoothing(
                        self.cluster_size, self.key_value_pairs_per_codebook, self.eps
                    ...
                    # keys_normalized.shape = self.keys_avg.shape = cjk
                    keys_normalized = current_keys_avg / cluster_size[:, :, None]
                    self.keys.data.copy_(keys_normalized)
                    self.expire_codes_(flatten) #VQ-VAE
                else:
                    if self.count_cluster_size:
                        ...
                        cluster_size = self.get_cluster_size(keys_ind)
                        # cluster_size = keys_onehot.sum(1)
                        self.cluster_size_counter.data.add_(cluster_size) #not sure if this part is correct
                # Remember: i = (b * n * topk)
                """ quantized_keys = rearrange(quantized_keys, "c (b n topk) k -> b c topk n k", b=b, n=n, topk=forward_topk)
                keys_ind = rearrange(keys_ind, "c (b n topk) -> b c topk n", b=b, n=n, topk=forward_topk)[:, :, :, :, None]
                quantized_values = rearrange(quantized_values, "c (b n topk) v -> b c topk n v", b=b, n=n, topk=forward_topk)
                ...
                all_quantized_values = self.update_tensor(all_quantized_values, quantized_values)
                #self.update_tensor(all_quantized_keys, quantized_keys)
                #self.update_tensor(all_keys_ind, keys_ind)
                #self.update_tensor(all_dist_to_fetched_keys, dist_to_fetched_keys)
                #self.update_tensor(all_counts, counts)
            #if self.return_values_only:
                
            return (all_quantized_values,)
            #else:
            #    return all_quantized_values, all_quantized_keys, all_keys_ind, all_dist_to_fetched_keys, all_counts
    
    def update_tensor(self, result, x: torch.Tensor):
        if result is None:
            result = x
        else:
...
class BottleneckedEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, labels: torch.Tensor):
        ...
        x, *extra_info = self.bottleneck(x_splitter, labels)
...
class SLPretrainedBottleneckedEncoder(BottleneckedEncoder):
    ...
    def __init__(
        ...
        print("init SLPretrainedBottleneckedEncoder")
        assert backbone in self.VALID_BACKBONES
        self.extracted_layer = extracted_layer
        encoder_is_channel_last = False
        transforms = None
